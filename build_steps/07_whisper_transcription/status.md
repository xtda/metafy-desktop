...
- Whisper MVP strategy: use `METAFY_WHISPER_CPP_PATH` when set, otherwise resolve `whisper-cli`, `main`, or `whisper` from `PATH`.
...
- Historical: `cargo test` from `src-tauri`: 9 passed, including fake local command transcription, raw JSON storage, and timestamped segment persistence with confidence.
